...
public:
  ...
  void show(int row, unsigned int proglen, unsigned int devlen, FILE *fp);
  void log();
...
void Line::show(int row, unsigned int proglen, unsigned int devlen, FILE *fp) {
  assert(m_pid >= 0);
...
void show_ncurses(Line *lines[], int nproc) {
 ...
 int devlen = get_devlen(lines, nproc, rows);
  ...
  for (i = 0; i < nproc; i++) {
    if (i + 3 < rows)
      lines[i]->show(i + 3, proglen,devlen, fp);
  ...
  fprintf(fp, "  TOTAL          %11.3f %11.3f ", sent_global, recv_global);
  fclose(fp);
